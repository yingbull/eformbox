function onLoad() {
	changeSRC();
	document.getElementById("ClearLab").click(); // remove lab req page 2 on initial load
	// new load of eFORM
	//optimize window size/width
	top.window.resizeTo(1190, screen.availHeight);

	if ($("#counter").val() == "1") {
		$("#counter").val(2);
	}
	// equivalent of setDefault, where value 1 = new form.  Counter starts off as null
	if ($("#counter").val() == "") {
		$("#counter").val(1);
		parseName("PhysicianName");
		parseName("PhysicianName2");
		parseName("PatientMD");
		//	checkIt();  // set default insurer to MSP
		$("#BillTo_1").val("X"); // set default insurer to MSP
		$("#BillTo2_1").val("X");
		//	date(); // get eGFR info - does not seem to work; swap with eGFRdate()
		eGFRdate(); // get eGFR info
		CheckCopyTo();
		PopValues();
		start(); // check conditions, including pregnant state
		copyMat();
	}
}
...
function completeCheck() {
	//This warns if boxes not checked
	if ($("#Ultrasound").val() == "X") {
		//if this is an US then no need to warn if boxes not checked
		wordCount();
	} else if (($("#PregnantYes").val() !== "X" && $("#PregnantNo").val() !== "X") || ($("#DiabeticYes").val() !== "X" && $("#DiabeticNo").val() !== "X") || ($("#AnticoagulantYes").val() !== "X" && $("#AnticoagulantNo").val() !== "X")) {
		return confirm("Please complete all of the check boxes\nbelow Relevant History.");
	} else if ($("#DiabeticYes").val() == "X" && $("#MetforminYes").val() !== "X" && $("#MetforminNo").val() !== "X") {
		return confirm("You have indicated the patient has diabetes.\nPlease indicate if they are taking Metformin.");
	} else {
		wordCount();
	}
}

function completeCheckSubmit() {
	if ($("#Ultrasound").val() == "X") {
		//if this is an US then no need to warn if boxes not checked
		wordCountSubmit();
	} else if (($("#PregnantYes").val() !== "X" && $("#PregnantNo").val() !== "X") || ($("#DiabeticYes").val() !== "X" && $("#DiabeticNo").val() !== "X") || ($("#AnticoagulantYes").val() !== "X" && $("#AnticoagulantNo").val() !== "X")) {
...
// This counts the words in the information box and sets an alert if less than a certain number
function wordCount() {
	word_count = $("#RelevantHistory").val().split(" ").length;
	if (word_count < 5) {
		var r = confirm("The radiologist requires as much information as possible to interpret the images.\nAre you sure that you have provided as much information as you can?\nClick OK to ignore this message.\nClick Cancel to return to the form to add more information.");
		if (r == true) {
			printForm();
		}
	} else {
...
function wordCountSubmit() {
	word_count = $("#RelevantHistory").val().split(" ").length;
	de = $("#submitDelay").val();
	if (!de) {
		$("#submitDelay").val("2000");
	}
	if (word_count < 5) {
		var r = confirm("The radiologist requires as much information as possible to interpret the images.\nAre you sure that you have provided as much information as you can?\nClick OK to ignore this message.\nClick Cancel to return to the form to add more information.");
		if (r == true) {
			var afterPrint = function () {
				delay = $("#submitDelay").val();
				setTimeout("SubmitButton.click()", delay);
			};
			if (window.matchMedia) {
				var mediaQueryList = window.matchMedia("print");
				function onPrintListener(mql) {
					if (mql.matches) {
					} else {
						afterPrint();
					}
				}
				mediaQueryList.addListener(onPrintListener);
			}
			...
			if (mediaQueryList) {
				mediaQueryList.removeListener(onPrintListener);
			}
		}
	} else {
		var afterPrint = function () {
...
function increaseFont() {
	var fontSize = parseInt($("[name=RelevantHistory]").css("font-size"));
	fontSize = fontSize + 1 + "px";
	$("[name=RelevantHistory]").css({ "font-size": fontSize });
}
...
function increaseFont2() {
	var fontSize = parseInt($("[name=RelevantHistoryText]").css("font-size"));
	fontSize = fontSize + 1 + "px";
	$("[name=RelevantHistoryText]").css({ "font-size": fontSize });
}
function decreaseFont2() {
	var fontSize = parseInt($("[name=RelevantHistoryText]").css("font-size"));
	fontSize = fontSize - 1 + "px";
	$("[name=RelevantHistoryText]").css({ "font-size": fontSize });
}
...
function changeFontSize(name, delta) {
	...
	element.css({ "font-size": oldSize + delta + "px" });
}
...
function showAll() {
	//this opens all the pages so if you open a saved form you can see them
	$("#page1").show();
	$("#page2").show();
	$("#page16").show();
}

//Lab engine to find DDimer
...
function PopDdimer(result) {
	DDIM = result.value;
	DDIMdate = new Date(result.date);
	$("#DD").val(DDIM);
	$("#DDDate").val(DDIMdate);
}
...
function checkIt() {
	// sets default insurer to MSP; simplified in initial onload
	if ($("#BillToWSBC").val() == "" && $("#BillToICBC").val() == "" && $("#BillToPatient").val() == "" && $("#BillToOther").val() == "") {
		$("#BillToMSP").val("X");
		$("#BillToMSP2").val("X");
	}
}
...
function CheckCopyTo() {
	...
	if (document.getElementById("PhysicianName").value != document.getElementById("PatientMD").value) {
		...
		document.getElementById("CopyTo2").value = document.getElementById("ReferralMD").value + "-" + document.getElementById("ReferralMD_MSP").value;
	}
}
...
function start() {
	...
	if (document.getElementById("DMT").value == "") {
		document.FormName.DiabeticNo.value = "X";
	}
	if (document.getElementById("DMT").value == "No" || document.getElementById("DMT").value == "Yes" || document.getElementById("DMT").value == "NotApplicable") {
		document.FormName.DiabeticYes.value = "X";
		document.FormName.DiabeticNo.value = "";
	}
	...
	var Drug = /METFORMIN|GLUMETZA|GLUCOPHAGE/i;
	var string1 = document.getElementById("Meds").value;
	var match = string1.search(Drug);
	if (match != -1) {
		document.FormName.MetforminYes.value = "X";
	} else {
		if ($("#DiabeticYes").val() == "X") {
			$("#MetforminNo").css("background-color", "yellow");
		}
	}
	...
	//Check if on anticoagulation
	var Drug2 = /WARFARIN|COUMADIN|DABIGATRAN|PRADAXA|RIVAROXABAN|XARELTO|APIXABAN|ELIQUIS|DALTEPARIN|FRAGMIN/i;
	var string2 = document.getElementById("Meds").value;
	var match = string2.search(Drug2);
	if (match != -1) {
		document.FormName.AnticoagulantYes.value = "X";
		document.getElementById("AnticoagulantText").placeholder = "add details";
		$("#AnticoagulantText").css("background-color", "yellow");
	} else {
		document.FormName.AnticoagulantNo.value = "X";
		document.getElementById("AnticoagulantText").placeholder = "";
		$("#AnticoagulantText").css("background-color", "transparent");
	}

	//Check if on dialysis
	if (document.getElementById("eGFRResult").value > 14 || !document.getElementById("eGFRResult").value) {
		document.FormName.DialysisNo.value = "X";
	}

	//Autochecks isolation standard and makes it yellow
	$("#IsolationStandard").val("X");
	$("#IsolationStandard").css("background-color", "yellow");
}

function copyMat() {
	//adds cc to Caseroom for all pregnant patients
	var a = $("#EDC").val();
	var b = $("#today").val();
	if (a > b) {
...
function undoCheck() {
	//If intervention is checked then this undoes the not on anticoagulant check box
	document.FormName.AnticoagulantNo.value = "";
}

function date() {
	// does not seem to work consistently; funny character &#164;
	var dateE = $("#eGFRDate").val();
	var dateEM = dateE.split(" ")[0];
	var eGFRValue = $("#eGFRResult").val();
	var CreatinineValue = $("#CreatinineResult").val();
	if (dateEM) {
		...
		if (diffDays < 91) {
			$("#eGFRDateD").val(dateEM);
			$("#eGFR").val(eGFRValue);
			$("#eGFRConfirm").html("&#164").prop("readonly", true);
			$("#eGFRDateD").prop("readonly", true);
			$("#eGFR").prop("readonly", true);
			$("#Creatinine").prop("readonly", true);
			$("#CreatinineDateD").prop("readonly", true);
			var dateC = $("#CreatinineDate").val();
			var dateCM = dateC.split(" ")[0];
			$("#CreatinineDateD").val(dateCM);
			$("#Creatinine").val(CreatinineValue);
		}
	}
}

function eGFRdate() {
	// simplified eGFR checker - checks for values older than 90 days and triggers lab req alert
	var startDate = document.getElementById("eGFRDateDate").value;
	var endDate = document.getElementById("today").value;
	var date1 = new Date(endDate);
	var date2 = new Date(startDate);
	var timeInMilisec = date1.getTime() - date2.getTime();
	var diffDays = Math.ceil(timeInMilisec / (1000 * 60 * 60 * 24));
	document.getElementById("eGFR_days").value = diffDays;
	if (diffDays > 90) {
		document.getElementById("labreq").style.background = "red";
		document.getElementById("eGFR_warning").style.background = "red";
	} else {
		document.getElementById("labreq").style.background = "lightskyblue";
		document.getElementById("eGFR_warning").style.background = "transparent";
	}
}
...
function bindPopup() {
	...
	$(function () {
		...
		$(".field").each(function () {
			//color empty demographic fields yellow
		...
		var edcDate = $("#EDC").val();
		if (!edcDate) {
			//This is for the servers that use EDD instead of EDC
			var edcDate = $("#EDD").val();
		}
		var LMP = $("#LMP").val();
		...
		var diffDays = Math.ceil(timeDiff / (1000 * 3600 * 24));
		if (diffDays >= -14 && diffDays <= 280) {
			//determines patient is pregnant if difference from -14 to 280
			$("#PregnantYes").val("X");
			$("#PregnantNo").val("");
		...
		$(".obDate", this)
			...
			.click(function () {
				...
				if (currentPreg) {
					...
					periodWeeks = $(this).val();
					actualPeriod = 40 - periodWeeks;
					periodDays = actualPeriod * 7; //recommendation is 18wk-22wk6d US so this choses 20 weeks
					...
					$("#RelevantHistory").val("For " + periodWeeks + " week US around " + desiredDateOutput);
					$("#PregnantNo").val("");
	...
	$(function () {
		//this opens up the needed pages
		var $divs = $(".divs > div");
		$divs.hide();
		$("#page2").hide();
		$("#page1").show();
	...
	// Intervention code
	$(function () {
		if ($("#AnticoagulantYes").val() == "X") {
			$("#BloodThinners1").css("color", "red");
			$("#BloodThinners2").css("color", "red");
			$("#BloodThinners3").css("color", "red");
			$("#BloodThinners4").css("color", "red");
			$("#BloodThinners5").css("color", "red");
			$("#BloodThinners6").css("color", "red");
		}
		var $divs = $(".divs2 > div");
		$(".int").click(function () {
			a = $(this).val();
			b = a.replace(/_/g, " "); //this gives the button value eg Peripheral injection
			c = $("#interventionSite").val();
			if (a == "Enter") {
				b = "";
			}
			d = b + " " + c; // this is the final output eg Peripheral Injection Knee Right
			e = $(this).attr("class");
			f = e.split(" ")[1]; //looks for site as second class eg arh
			g = e.split(" ")[2]; //looks for Rx as third class eg Rx
			if (f == "arh") {
				ARHCC();
			}
			if (f == "cgh") {
			...
			if (g == "NoRx") {
				$("#RelevantHistoryText").html(c);
			}
			if (g == "Other") {
				$("#page2").show(); // attaches lab form for eGFR
				$("#PatientInstructions2").val("When you get your booking you will be told if you need to go for this test and when to go.");
				document.getElementById("AdditionalInstructions2").style.fontSize = "13px";
				$("#AdditionalInstructions2").val("CBC, INR, PTT as required by imaging department and repeated as necessary.");
				$("#Diagnosis2").val("Interventional Procedure");
				$("#printer").val("C"); // used to print XR and Lab pages
				$("#submitDelay").val(3000);
				$("#RelevantHistoryText").html(c + "\n" + "Patient has been given a requisition for CBC, INR and PTT");
			}
			$("#ExamRequestedText").val(d);
			$("#SpecialProcedures").val("X");
			$(".modal-content").removeClass("active");
			$("#RelevantHistory").focus();
		});
		$(".mytags2").on("click", $(".boxIP"), function (evt) {
			findID(evt.target);
			function findID(element) {
				$("#interventionSite").val("");
				$(".boxIP").val(""); // only allows one selection
				if (element.val == undefined) {
					element = $(element); // Convert to jquery element
				}
				if (element.val() == "") {
				...
				site = element.attr("id"); // this gives the id of the checked box eg Knee_R
				siteInt = site.replace(/_/g, " ");
				imagesite = element.attr("class");
				imageSite = imagesite.split(" ")[2];
				bilateral = imagesite.split(" ")[3];
				if (imageSite == "arh") {
					ARHCC();
				}
				if (imageSite == "cgh") {
				...
				if (element.val() == "X") {
					$("#interventionSite").val(siteInt); //Sends id to hidden field for storage
				}
				if (bilateral == "hsg") {
					//this adds the HSG prep
					HSG();
				}
				c = $("#interventionSite").val();
				if (bilateral == "spine") {
					//this changes the prescription to the spinal one
					$("#RelevantHistoryText").html(c + "\n" + "Patient has been given a prescription for Triamcilone and Ropivacaine and a lab req for CBC,INR and PTT.");
					$("#PatientInstructions2").val("When you get your booking you will be told if you need to go for this test and when to go.");
					document.getElementById("AdditionalInstructions2").style.fontSize = "13px";
					$("#AdditionalInstructions2").val("CBC, INR, PTT as required by imaging department and repeated as necessary.");
					$("#Diagnosis2").val("Interventional Procedure");
					...
					$("#page17").hide();
					$("#printer").val("SP");
					$("#submitDelay").val(4000);
				}
				if (bilateral == "peripheral") {
					//this changes the prescription to the peripheral one
					$("#RelevantHistoryText").html(c + "\n" + "Patient has been given a prescription for Methylprednisolone and Ropivacaine.");
					$("#page17").show();
					$("#page16").hide();
					$("#printer").val("PE");
					$("#submitDelay").val(3000);
	...
	$(function () {
		...
		$(".XR").click(function () {
			...
			var g = "";
			if (d == "CN" && (r == "1" || r == "2")) {
				//If test to be done in 4 weeks or less
			...
			if (d == "CN" && (r == "3" || r == "4" || r == "N")) {
				//If there will be more than 4 weeks before test
			...
			o = n.replace(/_/g, " ");
			p = "";
			if (e !== "XR" && e != "DO" && e != undefined) {
				// DO indicates it is a doppler request
			...
			var ddre = 0;
			if (match != -1) {
				//This looks for prescence of < symbol which means the result is negative. Answer minus one means symbol not found
				var ddre = 1; // if ddre is 1 then then is a less than symbol so ddimer is negative.
			}
			if (e == "DO") {
				//indicates this is a doppler request
				...
				if (WellScore < 3 && WellScore > 0 && ((Ddimer < 500 && Ddimer >= 0 && Ddimer != "") || ddre == 1)) {
					$("#RelevantHistoryText")
						.html(o + "\n" + "Wells score: " + WellScore + " &nbsp&nbsp&nbsp" + WellInter + "\n" + "D-Dimer: " + Ddimer + " &nbsp&nbsp" + DDInter + "\n" + "Ongoing clinical concern despite the -ve D-dimer.")
						.prop("readonly", true);
				}
			} else {
				$("#RelevantHistoryText")
			...
			$("#GuideConfirm").html("&#164").prop("readonly", true); // special icon
			$("#RelevantHistory").focus();
		});
	});

	//This opens the grey imaging choices popup
	$(function () {
		...
		$(".mytags").on("click", $(".XBoxM"), function () {
			//This checks to see if there is a D-dimer result done within 4 days
			var d = $("#DD").val();
			var e = $("#DDDate").val();
			var dd = $("#DD").val();
			var regEx = /</g;
	...
	if ($("#counter").val() == "2") {
		//this only works if the form is being opened after submission
		var pages = $("#printer").val();
		if (pages == "C") {
			$("#page1").show();
			$("#page2").show();
